...
export async function loadPlannerData({ templeId, temple, budgetType, days, persons = 1 }) {
  if (!templeId) {
    throw new Error('Temple selection is required');
  }
  if (Number(days) <= 0) {
  ...
  if (plannerResponse?.status === 'error' || plannerResponse?.message?.includes('Invalid')) {
    throw new Error(plannerResponse.message || 'Invalid configuration payload');
  }
