...
export default function TravelPlannerPage() {
  ...
  const refreshPlanner = async (currentTemple = selectedTemple) => {
    ...
    try {
      ...
      setPlan(null);
      setError(err.message || 'Planner temporarily unavailable');
    } finally {
      setLoadingPlan(false);
    }
  };
  ...
  useEffect(() => {
    ...
    async function loadTemples() {
      ...
      try {
        const results = await getTemples();
        if (active) setTemples(results);
      } catch (err) {
        if (active) setError('Planner temporarily unavailable. Please verify connection.');
      } finally {
        if (active) setLoadingTemples(false);
      }
    }
    loadTemples();
    return () => { active = false; };
  ...
  useEffect(() => {
    let active = true;
    async function loadPlan() {
      if (active) await refreshPlanner(selectedTemple);
    }
    loadPlan();
    return () => { active = false; };
  }, [selectedTemple, budgetType, days]);
  ...
  const budgetOptions = plan?.budgetOptions || [];
  const timelineSteps = plan?.timeline || [];
  const nearbyPlaces = plan?.nearbyPlaces || [];
  const smartTips = plan?.smartTips || [];
  const riskNotes = plan?.riskNotes || [];
  ...
  return (
    <section className="section-shell py-12">
      <div className="max-w-4xl space-y-3">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-temple-gold">Smart Temple Travel Planner</p>
        <h1 className="display-font text-4xl font-bold leading-tight text-temple-deep sm:text-5xl">Intelligent pilgrimage assistant for temple journeys</h1>
        <p className="section-copy">Centralized intelligent coordinate tracking engine mapping backend routes, metrics and validation criteria dynamically.</p>
      </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="surface-card rounded-[2rem] p-5">
          ...
          <div className="mt-5 grid gap-4">
            ...
            {error && (
              <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 flex flex-col gap-2">
                <p>⚠️ {error}</p>
                <button type="button" onClick={() => refreshPlanner(selectedTemple)} className="text-xs font-bold uppercase tracking-wider text-red-900 underline text-left hover:text-red-950">
                  🔄 Retry Request
                </button>
              </div>
            )}
      ...
      {/* Conditional Layout Container: Renders beautifully when dataset is functional */}
      {plan ? (
        <>
          <div className="mt-10 grid gap-6 xl:grid-cols-2">
            <section>
              ...
              <div className="mt-4 grid gap-3">
                {routeOptions.length ? routeOptions.map((route) => (
                  <div key={route.id} className="surface-card rounded-2xl p-4 transition hover:-translate-y-0.5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-temple-gold">{route.mode}</p>
                        <h4 className="mt-1 font-semibold text-temple-deep">{route.source} to {route.destination}</h4>
                ...
                )) : <div className="surface-card rounded-2xl p-4 text-sm text-app-muted">No route paths matching criteria found.</div>}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="mt-4 grid gap-3">
                {budgetOptions.length ? budgetOptions.map((budget) => (
                  <div key={budget.id} className="surface-card rounded-2xl p-4 transition hover:-translate-y-0.5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-temple-gold">{budget.type.toUpperCase()}</p>
                        <h4 className="mt-1 font-semibold text-temple-deep">{formatMoney(budget.minCost, budget.maxCost)}</h4>
                      </div>
                    </div>
                  </div>
                )) : <div className="surface-card rounded-2xl p-4 text-sm text-app-muted">No pricing profiles matching constraints found.</div>}
              </div>
            </section>
          </div>
          ...
          <div className="mt-10 grid gap-6 xl:grid-cols-2">
            <section>
              ...
              <div className="mt-4 grid gap-3">
                {timelineSteps.length ? timelineSteps.map((step) => (
                  <div key={`${step.order}-${step.title}`} className="flex items-start gap-4 rounded-2xl border border-[color:var(--app-border)] bg-[color:var(--app-card)] px-4 py-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-temple-deep text-sm font-semibold text-white">{step.order}</div>
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-temple-gold">{step.title}</p>
                      <p className="mt-1 text-sm text-temple-ink">{step.detail}</p>
                    </div>
                  </div>
                )) : <div className="surface-card rounded-2xl p-4 text-sm text-app-muted">No recommendations found</div>}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="mt-4 grid gap-3">
                ...
                )) : <div className="surface-card rounded-2xl p-4 text-sm text-app-muted">No nearby places records matching selection found.</div>}
              </div>
            </section>
          </div>
        </>
      ) : (
        <div className="mt-8 surface-card rounded-[2rem] p-8 text-center text-app-muted">
          Adjust inputs and select "Generate Plan" to initialize tracking milestones.
        </div>
      )}
    </section>
  );
